...
object Finder {
  ...
  def combine[T](ctx: CaseClass[Typeclass, T]): Finder[T] = new Finder[T] {
    override def findById(id: String, tableDescription: EntityDesc)(implicit xa: doobie.Transactor[IO]): IO[Option[T]] = {
  ...
  implicit val intFinder = new Finder[Int] {
    override def findById(id: String, tableDescription: EntityDesc)(implicit xa: doobie.Transactor[IO]): IO[Option[Int]] =
      IO(Some(id.toInt))
  }

  implicit val stringFinder = new Finder[String] {
    override def findById(id: String, tableDescription: EntityDesc)(implicit xa: doobie.Transactor[IO]): IO[Option[String]] =
      IO(Some(id))
  }

  implicit val doubleFinder = new Finder[Double] {
    override def findById(id: String, tableDescription: EntityDesc)(implicit xa: doobie.Transactor[IO]): IO[Option[Double]] =
      IO(Some(id.toDouble))
  }
